      ...
      <div className='mt-5 mb-5 container'>
        ...
        <div className='mt-5 hotels'>
          {hotels.map((hotel, index) => {
            return (
              <Card style={{ width: '18rem', border: '1px solid #ddd' }} key={index}>
                <Card.Img variant='top' src={hotel.src} height={280} className='object-fit-cover' />
                <Card.Body style={{ flex: '0' }}>
                  <Card.Title className='text-description'>{hotel.name}</Card.Title>
                ...
                {hotel?.link &&
                  <Card.Body className='text-center' style={{ placeContent: 'center' }}>
                    <Button className='btn-reserve' href={hotel.link} target='_blank'>{t('hotels.book')}</Button>
                  </Card.Body>}
              </Card>
            )
          }
